...
export async function setupReserve(
  ...
  const initWethReserveTx = await _poolConfigurator.initLiquidityReserves(reserveParamsWeth);

  initWethReserveTx.wait();

  const initWethBorrowReserveTx = await _poolConfigurator.initVariableBorrowReserves(reserveParamsWeth);

  initWethBorrowReserveTx.wait();

  const initWethStableReserveTx = await _poolConfigurator.initStableBorrowReserves(reserveParamsWeth);

  initWethStableReserveTx.wait();
  ...
  await _poolConfigurator.setReserveBorrowing(wethAddress, true)


  const avasCollateralUnderlyingAddress = await _aaveProtocolDataProvider.getReserveTokensAddresses(wethAddress);
  ...
  const colAvasTokenAggregatorAddress = await _aaveProtocolDataProvider.getAggregatorAddress(wethAddress);
  ...
  const WethStandardStrategy = await ethers.getContractFactory("StandardStrategy");
  const _wethStandardStrategy = await WethStandardStrategy.deploy(
  ...
  const wethAggregator = await ethers.getContractFactory("Aggregator")

  const _wethAggregator = await wethAggregator.attach(colAvasTokenAggregatorAddress);

  await _wethAggregator.setAggregatorStrategy(_wethStandardStrategy.address)
  ...
  await _emissionManager.connect(owner).configureAssets(wethRewardsConfigInput)
