...
export async function withdraw_router(
    provider,
    borrower,
    borrowerAddress,
  ...
  console.log("withdraw_router underlyingBorrowDecimals", underlyingBorrowDecimals);
  ...
  // Borrow Underlying
  const repayAmount = ethers.utils.parseUnits("1000", underlyingDepositDecimals);
  ...
  const tx = await _pool.connect(borrower).withdrawRouter(
    underlyingDepositAddress,
    routerAddress,
    repayAmount,
    borrowerAddress
