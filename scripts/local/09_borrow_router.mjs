...
export async function borrow_router(
    provider,
    borrower,
    borrowerAddress,
  ...
  console.log("borrow_router _pool", _pool.address);

  // Borrow Underlying
  const borrowAmount = ethers.utils.parseUnits("100", underlyingBorrowDecimals);

  await _underlyingBorrowErc20.connect(borrower).approve(_pool.address, borrowAmount);
  // await _underlyingBorrowErc20.connect(borrower).mint(borrowAmount);
  await _underlyingBorrowErc20.connect(borrower)["mint(uint256)"](borrowAmount)

  const tx = await _pool.connect(borrower).borrowRouter(
    underlyingBorrowAddress, // usdc
    routerAddress,
    borrowAmount,
    "2",
    borrowerAddress
