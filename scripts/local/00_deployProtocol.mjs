...
export async function deployProtocol(
  ...
  await _priceOracle.setAssetPrice(underlyingAddress, ethers.utils.parseUnits("1.0", 8));
  await _priceOracle.setAssetPrice(wethAddress, ethers.utils.parseUnits("1000", 8));
  ...
  await _aclManager.addRiskAdmin(ownerAddress);
  await _aclManager.addAssetListingAdmin(ownerAddress);
  //
  ...
  // const _poolDataProvider = await PoolDataProvider.deploy(_poolAddressesProvider.address);
  // await _poolDataProvider.deployed();
  // await _poolAddressesProvider.setPoolDataProvider(_poolDataProvider.address);
  //
  ...
  // const _allocator = await Allocator.deploy(
  //   _poolAddressesProvider.address,
  //   [underlyingAddress, wethAddress],
  //   "1", // votesToPass_
  //   "1", // votesToFail_,
  //   "100" // allocatorSubmitTimeRequirement_
  // );
  // await _allocator.deployed();

  // await _allocator.setValidator(validatorAddress);
  // await _allocator.setAllocator(allocatorCallerAddress);
  // console.log("allocatorCallerAddress", allocatorCallerAddress);
