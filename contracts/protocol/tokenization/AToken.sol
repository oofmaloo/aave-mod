...
contract AToken is ScaledBalanceTokenBase, EIP712Base, IAToken {
  ...
  /// increase allowance
  function initAggregator() external override {
      //
  }
  ...
  function mintToRewards(uint256 amount, uint256 index) external override onlyPool {
    if (amount == 0) {
      return;
    }
    address dividendsVault = POOL.getDividendsVaultAddress(_underlyingAsset);
    _mintScaled(address(POOL), dividendsVault, amount, index);
  }
  ...
  function balanceOf(address user)
    ...
    returns (uint256)
  {
    console.log("AToken balanceOf", address(POOL));
