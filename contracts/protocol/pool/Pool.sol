...
contract Pool is PoolStorage, IPool {
  ...
  function finalizeTransfer(
    ...
    require(
      (
        ...
        msg.sender == _reserves[asset].vaultAddress
      ) &&
  ...
  function setAToken(address asset, address aTokenAddress) external override onlyPoolConfigurator {
    _reserves[asset].aTokenAddress = aTokenAddress;
  }

  function setVaultToken(address asset, address vaultTokenAddress) external override onlyPoolConfigurator {
    _reserves[asset].vaultAddress = vaultTokenAddress;
  }

  function setVariableDebtToken(address asset, address variableDebtTokenAddress) external override onlyPoolConfigurator {
    _reserves[asset].variableDebtTokenAddress = variableDebtTokenAddress;
  }

  function setStableDebtToken(address asset, address stableDebtTokenAddress) external override onlyPoolConfigurator {
    _reserves[asset].stableDebtTokenAddress = stableDebtTokenAddress;
  }

  function setAggregator(address asset, address aggregatorAddress) external override {
    _reserves[asset].aggregatorAddress = aggregatorAddress;
  }

  function setInterestRateOracle(address asset, address interestRateOracleAddress) external override {
    _reserves[asset].interestRateOracleAddress = interestRateOracleAddress;
  }

  function setDividendsVaultAddress(address asset, address newDividendsVaultAddress) external override onlyPoolConfigurator {
    _reserves[asset].dividendsVaultAddress = newDividendsVaultAddress;
  }

  function getDividendsVaultAddress(address asset) external view override returns (address) {
    return _reserves[asset].dividendsVaultAddress;
  }
