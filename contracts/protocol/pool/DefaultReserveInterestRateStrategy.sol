...
contract DefaultReserveInterestRateStrategy is IReserveInterestRateStrategy {
  ...
  function getBaseDebtRate(address aggregator, uint256 baseRate) public view returns (uint256) {
    console.log("getBaseDebtRate baseRate", baseRate);
    address aggregatorStrategy = IAggregator(aggregator).getAggregatorStrategy();
    console.log("aggregatorStrategy", aggregatorStrategy);
    address interestRateOracle = IStandardStrategy(aggregatorStrategy).getInterestRateOracle();
    console.log("interestRateOracle", interestRateOracle);
    uint256 avgBorrowRate = IInterestRateOracleV2(interestRateOracle).getLastAverageBorrowRate(aggregator);
    console.log("avgBorrowRate", avgBorrowRate);
    return baseRate < avgBorrowRate ? baseRate : avgBorrowRate;
  }
    ...
    )
  {
    ...
    console.log("calculateInterestRates totalDebt", vars.totalDebt );

    vars.currentLiquidityRate = 0;
    // vars.currentVariableBorrowRate = _baseVariableBorrowRate;
    // vars.currentVariableBorrowRate = params.baseRate;
    vars.currentVariableBorrowRate = getBaseDebtRate(params.aggregator, params.baseRate);
    vars.currentStableBorrowRate = getBaseStableBorrowRate();
    console.log("calculateInterestRates currentStableBorrowRate", vars.currentStableBorrowRate );

    if (vars.totalDebt != 0) {
      vars.stableToTotalDebtRatio = params.totalStableDebt.rayDiv(vars.totalDebt);
      console.log("calculateInterestRates stableToTotalDebtRatio", vars.stableToTotalDebtRatio );
      ...
      // vars.availableLiquidity =
      //   IAggregator(params.aggregator).getBalanceStored() +
      //   params.liquidityAdded -
      //   params.liquidityTaken;

      vars.availableLiquidity =
        IAggregator(params.aggregator).balance() +
        params.liquidityAdded -
        params.liquidityTaken;
        
      console.log("calculateInterestRates availableLiquidity", vars.availableLiquidity );

      vars.availableLiquidityPlusDebt = vars.availableLiquidity + vars.totalDebt;
      console.log("calculateInterestRates availableLiquidityPlusDebt", vars.availableLiquidityPlusDebt );
      vars.borrowUsageRatio = vars.totalDebt.rayDiv(vars.availableLiquidityPlusDebt);
      console.log("calculateInterestRates borrowUsageRatio", vars.borrowUsageRatio );
