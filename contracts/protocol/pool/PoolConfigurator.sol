...
import {ATokenConfiguratorLogic} from '../libraries/logic/ATokenConfiguratorLogic.sol';
import {VariableDebtConfiguratorLogic} from '../libraries/logic/VariableDebtConfiguratorLogic.sol';
import {StableDebtConfiguratorLogic} from '../libraries/logic/StableDebtConfiguratorLogic.sol';
import {VaultTokenConfiguratorLogic} from '../libraries/logic/VaultTokenConfiguratorLogic.sol';
...
contract PoolConfigurator is IPoolConfigurator {
  ...
  // ITokenizationHelper internal _tokenizationHelper;
  ...
  function initLiquidityReserves(ConfiguratorInputTypes.InitReserveInput[] calldata input)
    external
    override
    onlyAssetListingOrPoolAdmins
  {
    ...
    for (uint256 i = 0; i < input.length; i++) {
      ATokenConfiguratorLogic.executeInitLiquidityReserve(cachedPool, input[i]);
    }
  }

  function initVaultReserves(ConfiguratorInputTypes.InitReserveInput[] calldata input)
    external
    override
    onlyAssetListingOrPoolAdmins
  {
    IPool cachedPool = _pool;
    for (uint256 i = 0; i < input.length; i++) {
      VaultTokenConfiguratorLogic.executeInitVaultTokenReserve(cachedPool, input[i]);
    }
  }

  function initVariableBorrowReserves(ConfiguratorInputTypes.InitReserveInput[] calldata input)
    external
    onlyAssetListingOrPoolAdmins
  {
    IPool cachedPool = _pool;
    for (uint256 i = 0; i < input.length; i++) {
      VariableDebtConfiguratorLogic.executeInitVariableBorrowReserve(cachedPool, input[i]);
    }
  }

  function initStableBorrowReserves(ConfiguratorInputTypes.InitReserveInput[] calldata input)
    external
    onlyAssetListingOrPoolAdmins
  {
    IPool cachedPool = _pool;
    for (uint256 i = 0; i < input.length; i++) {
      StableDebtConfiguratorLogic.executeInitStableBorrowReserve(cachedPool, input[i]);
    }
  }
  ...
  //   IPool cachedPool = _pool;
  //   for (uint256 i = 0; i < input.length; i++) {
  //     ConfiguratorLogic.executeInitLiquidityReserve(cachedPool, _tokenizationHelper, input[i]);
  //   }
  // }
  ...
  //   IPool cachedPool = _pool;
  //   for (uint256 i = 0; i < input.length; i++) {
  //     ConfiguratorLogic.executeInitVariableBorrowReserve(cachedPool, _tokenizationHelper, input[i]);
  //   }
  // }
  ...
  //   IPool cachedPool = _pool;
  //   for (uint256 i = 0; i < input.length; i++) {
  //     ConfiguratorLogic.executeInitStableBorrowReserve(cachedPool, _tokenizationHelper, input[i]);
  //   }
  // }
  ...
  function setReserveBorrowing(address asset, bool enabled) external override onlyRiskOrPoolAdmins {
    ...
    if (!enabled) {
      ...
      require(
        reserve.variableDebtTokenAddress != address(0) &&
        reserve.stableDebtTokenAddress != address(0), 
        "Error, Debt tokens not initialized"
      );
    }
  ...
  // function setReserveStableBorrowing(address asset, bool enabled) external override onlyRiskOrPoolAdmins {
  //   DataTypes.ReserveConfigurationMap memory currentConfig = _pool.getConfiguration(asset);
  //   if (!enabled) {
  //     require(!currentConfig.getStableRateBorrowingEnabled(), Errors.STABLE_BORROWING_ENABLED);
  //     DataTypes.ReserveData memory reserve = _pool.getReserveData(asset);
  ...
  // function setReserveVariableBorrowing(address asset, bool enabled) external override onlyRiskOrPoolAdmins {
  //   DataTypes.ReserveConfigurationMap memory currentConfig = _pool.getConfiguration(asset);
  //   if (!enabled) {
  //     require(!currentConfig.getStableRateBorrowingEnabled(), Errors.STABLE_BORROWING_ENABLED);
  //     DataTypes.ReserveData memory reserve = _pool.getReserveData(asset);
  //     require(
  //       reserve.variableDebtTokenAddress != address(0),
  //       "Error, Varible Debt token not initialized"
  //     );
  //   }
  //   currentConfig.setBorrowingEnabled(enabled);
  //   _pool.setConfiguration(asset, currentConfig);
  ...
  function setRewardFactor(address asset, uint256 newRewardFactor)
    external
    override
    onlyRiskOrPoolAdmins
  {
    DataTypes.ReserveConfigurationMap memory currentConfig = _pool.getConfiguration(asset);
    uint256 oldRewardFactor = currentConfig.getRewardFactor();
    currentConfig.setRewardFactor(newRewardFactor);
    _pool.setConfiguration(asset, currentConfig);
    // emit UnbackedMintCapChanged(asset, oldRewardFactor, newRewardFactor);
  }

  function setDividendsVaultAddress(address asset, address newDividendsVaultAddress)
    external
    override
    onlyRiskOrPoolAdmins
  {
    _pool.setDividendsVaultAddress(asset, newDividendsVaultAddress);
    // emit UnbackedMintCapChanged(asset, oldRewardFactor, newRewardFactor);
  }
