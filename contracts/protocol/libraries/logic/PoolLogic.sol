...
library PoolLogic {
  ...
  function executeMintToTreasury(
    ...
    for (uint256 i = 0; i < assets.length; i++) {
      ...
      console.log("executeMintToTreasury start");
      ...
      console.log("executeMintToTreasury accruedToTreasury", accruedToTreasury);

      if (accruedToTreasury != 0) {
        reserve.accruedToTreasury = 0;
        uint256 normalizedIncome = reserve.getNormalizedIncome();
        uint256 amountToMint = accruedToTreasury.rayMul(normalizedIncome);
        console.log("executeMintToTreasury amountToMint", amountToMint);

        uint256 rewardFactor = reserve.configuration.getRewardFactor();
        console.log("executeMintToTreasury rewardFactor", rewardFactor);

        uint256 amountToReward;
        if (rewardFactor != 0) {
          amountToReward = amountToMint.percentMul(PercentageMath.PERCENTAGE_FACTOR - rewardFactor);
          console.log("executeMintToTreasury amountToReward", amountToReward);
          amountToMint = amountToMint - amountToReward;
          console.log("executeMintToTreasury amountToMint", amountToMint);

          IAToken(reserve.aTokenAddress).mintToRewards(amountToReward, normalizedIncome);
        }
