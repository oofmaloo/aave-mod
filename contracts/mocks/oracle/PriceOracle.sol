...
contract PriceOracle is IPriceOracle {
  ...
  mapping(address => uint256) internal ethPrices;
  ...
  event AssetEthPriceUpdated(address asset, uint256 price, uint256 timestamp);
  ...
  // asset/eth price
  function getAssetEthPrice(address asset) external view override returns (uint256) {
    return ethPrices[asset];
  }

  // asset/eth price
  function setAssetEthPrice(address asset, uint256 price) external override {
    ethPrices[asset] = price;
    emit AssetEthPriceUpdated(asset, price, block.timestamp);
  }
